...
$(document).ready(function(){
	...
	$('.goa, .sik, .nag, .tri, .mani, .assam, .wb, .jha, .chh, .ori, .tam, .kar, .raj, .anp, .mah, .mp, .ker, .pun, .jk, .hp, .arp, .har, .utr, .bih, .meg, .miz, .guj').dblclick( //double click listener
		function () {
	...
	$('area, #regions li').hover( //hover listener
		...
		function() { //if the user hovers over an <area> element or a <li> in <ul class="regions">...
			...
			$('#map').attr("src","images/map-"+ area +"-mo.jpg");  //change the image of the <area> to the mouseover version
			$('.'+area).css("background-color","#046baa") //change the background color of the corresponding area in the list
		;},
		...
		function() { //if the mouse leaves an <area> element or a <li> in <ul class="regions">...
			...
			$('#map').attr("src","images/map.jpg"); //change the image of the <area> back to default
			$('.'+area).css("background-color","#01588d"); //change the background color of the corresponding area in the list back to default
			}
	)
